...
class RNN_Phoneme_Classifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(0, x.shape[1]):
            ...
            for i in range(len(self.rnn)):
                x_t = self.rnn[i](x_t, self.hiddens[-1][i])
                hidden[i] = x_t
            self.hiddens.append(hidden.copy())
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(self.x.shape[1]-1, -1, -1):
            for i in range(len(self.rnn)-1, -1, -1):
                if i != 0:
                    h_prev_l = self.hiddens[t+1][i-1]
                else:
                    h_prev_l = self.x[:,t,:].reshape(self.x.shape[0],self.x.shape[2])
                dx_i, dh_i = self.rnn[i].backward(dh[i], self.hiddens[t+1][i], h_prev_l, self.hiddens[t][i])
                dh[i] = dh_i
                if i != 0:
                    dh[i-1] += dx_i
