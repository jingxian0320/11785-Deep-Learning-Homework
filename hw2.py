...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [Conv1D(x[0], x[1], x[2], x[3], conv_weight_init_fn,bias_init_fn) for x in zip([num_input_channels] + num_channels[:-1], num_channels, kernel_sizes, strides)]
        self.flatten = Flatten()
        conv_neurons = input_width
        for layer in self.convolutional_layers:
            conv_neurons = (conv_neurons-layer.kernel_size)//layer.stride+1
        output_width = conv_neurons * num_channels[-1]
        self.linear_layer = Linear(output_width,num_linear_neurons,linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.convolutional_layers):
            z = layer(self.output)
            self.output = self.activations[i](z)
